...
private class Scanner(packageName: String) extends Scanning(packageName) with LazyLogging:
  ...
  def allMethodsOf( cls : Class[_]): Try[MethodInfoList] = allMethodsOf(cls.getName)

  def allMethodsOf(className: String): Try[MethodInfoList] = using {
    scanResult =>
      scanResult.getClassInfo(className).getMethodInfo
  }
  ...
  end allLambdasDefiningAComparator
